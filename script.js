...
const veryOldInventors = inventors.filter((inv) => inv.year > 1500 && inv.year < 1600);
...
let inventorNames = [];
let mappedFirstNames = inventors.map((n) => inventorNames.push( {['first']: n.first, ['last']: n.last}));
...
//sort always converts the elements into strings, comparing their sequences of UTF-16 code units values
//so inorder to sort a numeric value, we need to proive it with a sorting function like return a and b in ascending order
//if the condition a - b is true
let sortedByBirthYear = inventors.toSorted((furthest, recent) => furthest.year - recent.year);
...
let inventorNamedAda = inventors.find((name) => name.first === 'Ada');
...
let isAdultPresent = devs.some((dev18YearOld) => 2024 - dev18YearOld.year >= 18);
...
let isEveryone19OrOlder = devs.every((person19YrsOld) => 2024 - person19YrsOld.year >= 19);
